...
class Shop extends Home_Controller {
	...
	public function likeSearch(){
		...
		foreach ($order_info as $key => $value) {
			if($order[$order_number]['order_id']!=$value['order_id']){
				...
				$map2['uid']=$value['uid'];
				$order[$order_number]['coupon']=$this -> coupon -> getInfo($map2);
				$product_number=0;
			}
			$order[$order_number]['order'][$product_number]=$value;
			$map3['id']=$value['product_id'];
			$product = $this -> product -> getInfo($map3,'',false);
			if($product != NULL){
	...
	public function order_list(){
		...
		$order_info=$this-> order -> getData($map, '', $order_sort);
		$order_number=0;
		$product_number=0;
		...
		foreach ($order_info as $key => $value) {
			if($orderList[$order_number]['order_id']!=$value['order_id']){
				...
				$map2['uid']=$value['uid'];
				$orderList[$order_number]['coupon']=$this -> coupon -> getInfo($map2);
				$product_number=0;
			}
	...
	public function order_info(){
		...
		foreach ($data as $key => $value) {
			...
			$order['order'][$key]['product']=$this-> product -> getInfo($map2);
			$total_price+=$value['price'];
		}
		$map3['uid']=$order['uid'];
		$order['coupon']=$this -> coupon -> getInfo($map3);
	...
	public function payLog()
	{
		...
		$map['paid'] = 1;
		$map['shop_id']=$shopadmin['shop_id'];
		$pay_log = $this -> order -> getData($map);
	...
	public function baggedChange(){
		$shopadmin=self::check_login();
		$map['order_id']=$this -> input -> get('order_id');
		$order=$this -> order -> getInfo($map);
		$order_change['bagged']=1-$order['bagged'];
		$result=$this -> order -> editMultiData($map,$order_change);
		if(sizeof($result)==0){
			echo "fail";
		}else{
			error_log(date('YmdHis')."-[change order:".$map['order_id']." bagged to ".$order_change['bagged']."]"."\n",3,"/tmp/agent_operate.log");
			echo "success";
		}
	}
	...
	public function pickedupChange(){
		...
		if(sizeof($result)==0){
			...
			error_log(date('YmdHis')."-[change order:".$map['order_id']." pickedup to ".$order_change['pickedup']."]"."\n",3,"/tmp/agent_operate.log");
			echo "success";
		}
	}
	...
	public function paidChange(){
		...
		if(sizeof($result)==0){
			...
			error_log(date('YmdHis')."-[change order:".$map['order_id']." paid to ".$order_change['paid']."]"."\n",3,"/tmp/agent_operate.log");
			echo "success";
		}
	}
	...
	public function couponChange(){
		...
		if(!$result){
			...
			error_log(date('YmdHis')."-[change uid:".$map['uid']." coupon_status to ".$data['coupon_status']."]"."\n",3,"/tmp/agent_operate.log");
			echo "success";
		}
	}
	...
	public function inventory_list(){
		...
		foreach ($data as $key => $value) {
			...
			$map2['order_status']=1;
			$inventory[$key]['ordered']=$this -> order -> count($map2);
			$map2['order_status']=2;
			$inventory[$key]['pickedup']=$this -> order -> count($map2);
