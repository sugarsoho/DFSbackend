	...
	public function order_list(){
		...
		if($order_info){
			...
			foreach ($order_info as $key => $value) {
				...
				$product = $this -> product -> getInfo($map2,'',false);
				if($product != NULL){
	...
	public function test(){
		...
		if($order_info){
			...
			foreach ($order_info as $key => $value) {
				...
				$order[$order_number]['order'][$product_number]=$value;
				$map2['id']=$value['product_id'];
				$order[$order_number]['order'][$product_number]['product']=$this -> product -> getInfo($map2,'',false);
				$product_number += 1;
			}
			unset($order[0]);
			echo json_encode(array_values($order));
