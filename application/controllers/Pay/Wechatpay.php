...
class Wechatpay extends CI_Controller {
	...
	public function callPay(){
		...
		if($uid==NULL) exit('未授权登录，请重新打开小程序');
		...
		$pay_body = array('openid' => $user_info['openId'],
											...
											'currency' => 'USD',
											'reference' => $order_id,
											'ipn_url' => 'http://139.199.170.78/Pay/Wechatpay/notify_pay');
	...
	public function getPayParameters($pay_body){
		$url = 'https://citconpay.com/payment/pay_wxmini';
		$header = array('Authorization: Bearer 404DE99B594841C59F524F16F64E1686');
		...
		if($data['transaction_id'] == ' '||$data['transaction_id'] == ''){
			...
			if ($error == NULL){
				...
				if($result != false){
					...
					$error1 = $this -> order -> editMultiData($map, $order_change);
					if ($error1 == NULL){
	...
	public function getRefundParameters($refund_body){
		$url = 'https://citconpay.com/payment/refund';
		$header = array('Authorization: Bearer 404DE99B594841C59F524F16F64E1686');
	...
	public function getCoupon($amount, $reference)
	{
		if($amount >= 12000){
			...
			$map1['uid'] = $order['uid'];
			$coupon = $this -> coupon -> getInfo($map1);
			$coupon['coupon_status'] = 1;
			$result = $this -> coupon -> editData($coupon);
			return true;
		}
		return false;
	}
