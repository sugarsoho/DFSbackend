...
class Order extends CI_Controller {
	...
	public function addCart(){
	    ...
	    $uid=$this-> input -> get('uid');
			...
			//获取库存及限购数目
			$product_info=$this-> product -> getInfo('product', $map);
	    ...
	    $result=$this -> order -> addData('order', $data);
	    if($result){
	    	echo 'success';
	    }
	    else{
	...
	public function changeCartNumber()
	{
		...
		$map['id'] = $id;
		$order_info = $this -> order -> getInfo('order', $map);
		if($option == -1) {
			$order_info['number'] -= 1;
			$order_info['price'] = $order_info['price'] * ($order_info['number'] / ($order_info['number'] + 1));
		}
		else if($option == 1){
			$order_info['number'] += 1;
			$order_info['price'] = $order_info['price'] * ($order_info['number'] / ($order_info['number'] - 1));
		...
		$result = $this -> order -> editData('order', $order_info);
	...
	public function confirmOrder(){
      ...
      $map['order_status']=0;
			...
			foreach ($order_info as $key => $value) {
				...
				$product_info=$this-> product -> getInfo('product', $map2);
      ...
      $result=$this-> order -> editMultiData('order', $map, $order_status);
      if($result){
        echo 'success';
      }
      else{
